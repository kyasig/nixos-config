{config, pkgs, inputs, lib, nix-colors,font,...}:
{
  programs.xmobar = {
    ...
    , template = "  %UnsafeXMonadLog%  }{ <fc=#${config.colorScheme.palette.base0D}></fc>  %memory%   <fc=#${config.colorScheme.palette.base0B}></fc>  %cpu%   <fc=#${config.colorScheme.palette.base08}></fc>  %uptime%   <fc=#${config.colorScheme.palette.base0A}>󰒋</fc>  %disku%   <fc=#${config.colorScheme.palette.base0C}></fc>  %battery%   <fc=#${config.colorScheme.palette.base0E}>󰥔</fc>  %date%  "
}
    '';
  };
}
